...
full_dim       = 1560
mask_second_half = mask[780:1560]    
cov            = config.cov[0:full_dim, 0:full_dim]
cov_inv        = np.linalg.inv(config.cov[0:full_dim, 0:full_dim])
cov_inv_masked = np.linalg.inv(config.cov[780:1560, 780:1560][mask_second_half][:,mask_second_half])
...
dv_predict = tmp # add cs and 2x2 part
...
for i in range(len(dv_predict)):
    chi2 = get_chi2(dv_predict[i], dv_validation[i], mask[780:1560], cov_inv_masked)
