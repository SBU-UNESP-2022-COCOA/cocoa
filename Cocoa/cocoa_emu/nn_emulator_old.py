...
class NNEmulator:
    ...
    def train(self, X, y, X_validation, y_validation, test_split=None, batch_size=32, n_epochs=100):
        ...
        np.savetxt("test_dv.txt", np.array( [tmp_Y_validation.cpu().detach().numpy()[-1], Y_pred.cpu().detach().numpy()[-1]] ), fmt='%s')
        self.trained = True
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            X_mean = self.X_mean.clone().detach().to(self.device)
            X_std  = self.X_std.clone().detach().to(self.device)
