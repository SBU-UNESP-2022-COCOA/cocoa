...
class NNEmulator:
    def __init__(self, N_DIM, OUTPUT_DIM, dv_fid, dv_std, cov_inv, dv_max, device, model='resnet', optim=None):
        ...
        if model == 'simply_connected':
            ...
            print("testing:", nn.Linear(N_DIM, 512), type(nn.Linear(N_DIM, 512)))
    ...
    def train(self, X, y, X_validation, y_validation, test_split=None, batch_size=32, n_epochs=100):
        ...
        np.savetxt("test_dv.txt", np.array( [tmp_Y_validation.cpu().detach().numpy().astype(np.float64)[-1], Y_pred.cpu().detach().numpy().astype(np.float64)[-1], ((tmp_X_validation - tmp_X_mean)/tmp_X_std).cpu().detach().numpy().astype(np.float64)[-1] ]), fmt='%s')
        self.trained = True
