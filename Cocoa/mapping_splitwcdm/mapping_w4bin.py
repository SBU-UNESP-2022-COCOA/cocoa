...
wbin_chains_root    = "./mapping/data/chains/EXAMPLE_MCMC8"
...
samples = loadMCSamples(wbin_chains_root,settings=analysissettings)
...
for i in range(len(logA)):
	info_wbin = {'likelihood': {'lsst_y1.lsst_cosmic_shear':
								...
								'non_linear_emul': 5, #Using wbin emulators
								'kmax_boltzmann': 5.0}},
...
np.savetxt('./mapping_splitwcdm/mapping_w4bin_'+str(job_idx)+'.txt', np.transpose([w0, w1, w2, w3, omm_geo, omm_growth, w_geo, w_growth, logA_mini, ns_mini]), fmt='%f')