...
with open("./mapping_splitwcdm/mapping_lcdm_0_"+str(end_idx)+".txt",'w') as output:
    for i in range(end_idx+1):
        with open("./mapping_splitwcdm/mapping_lcdm_"+str(i)+".txt",'r') as input:
            for line in input:
                output.write(line)

samples = np.loadtxt("./mapping_splitwcdm/mapping_lcdm_0_"+str(end_idx)+".txt")
print("checking shape: ",np.shape(samples))


omm_geo        = samples[:,0]
omm_growth     = samples[:,1]
w_geo          = samples[:,2]
w_growth       = samples[:,3]
...
# ### ========= Second Plot ========= ###
plt.figure().clear()
plt.scatter(pc_geo, pc_growth,label=r'Best fit of LCDM', s = 2)
...
plt.legend()
plt.savefig("./mapping_splitwcdm/mapping_lcdm_v2.pdf")
