...
### Read w0wa parameters and generate a temporary data vector

samples = loadMCSamples(w0wa_chains_root,settings=analysissettings2)
...
for i in range(len(logA)):
	info_w0wa = {'likelihood': {'lsst_y1.lsst_cosmic_shear':
								...
								'non_linear_emul': 3, #Using w0wa emulators
								'kmax_boltzmann': 5.0}},
	...
	### Adding param info
	info_w0wa["params"] = { 'logA':   {'value': logA[i], 'drop': True},
...
np.savetxt('./mapping_splitwcdm/mapping_w0wa_'+str(job_idx)+'.txt', np.transpose([w0, wa, omm_geo, omm_growth, w_geo, w_growth, logA_mini, ns_mini]), fmt='%f')