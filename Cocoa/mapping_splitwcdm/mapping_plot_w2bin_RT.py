...
with open("./mapping_splitwcdm/mapping_w2bin_RT_0_"+str(end_idx)+".txt",'w') as output:
    for i in range(end_idx+1):
        with open("./mapping_splitwcdm/mapping_w2bin_RT_"+str(i)+".txt",'r') as input:
            for line in input:
                output.write(line)

samples = np.loadtxt("./mapping_splitwcdm/mapping_w2bin_RT_0_"+str(end_idx)+".txt")
...
# ### ========= Second Plot ========= ###
plt.figure().clear()
plt.scatter(pc_geo, pc_growth,label=r'Best fit of 2 w-bin (Roman)', s = 2)
