...
class Minimize(Minimizer, CovmatSampler):
    ...
    def initialize(self):
        ...
        num_starts = int(np.ceil(self.best_of))#KZ
        if self.output:
    ...
    def process_results(self, results, successes, affine_transform_baselines,
        ...
        #KZ being: do not transform tuple to list
        #print("DEBUGGING--MINIMIZER1", type(results), results)
        results = list(chain(*results))
        #print("DEBUGGING--MINIMIZER2", type(results), results)
        ...
        #KZ: following reproduction check seems to cause some problems
    ...
    def check_force_resume(cls, output, info=None):
        ...
        if output.is_resuming():
            ...
            #                     "('-f', '--force', 'force: True')")
            # KZ
            print("Please use normal minimize with cpbaya MPI implementation")
            quit()
