...
with open("./mapping/mapping_w4bin_0_"+str(end_idx)+".txt",'w') as output:
    for i in range(end_idx+1):
        with open("./mapping/data/4wbin/mapping_w4bin_"+str(i)+".txt",'r') as input:
            for line in input:
                output.write(line)

samples = np.loadtxt("./mapping/mapping_w4bin_0_"+str(end_idx)+".txt")
...
# ### ========= Second Plot ========= ###
plt.figure().clear()
plt.scatter(omm_geo, omm_growth,label=r'Best fit of 4 w-bin', s = 2)
...
plt.legend()
plt.savefig("./mapping/mapping_4wbin_v2.pdf",bbox_inches='tight')
...
plt.savefig("./mapping/mapping_4wbin_v3.pdf",bbox_inches='tight')
